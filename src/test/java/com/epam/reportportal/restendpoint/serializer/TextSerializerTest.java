...
public class TextSerializerTest {
	...
	public void testContentType() {
		...
		SmartAssert.assertSoft(serializer.canRead(MediaType.ANY_TEXT_TYPE, byte[].class),
				CoreMatchers.is(true),
				"Wrong result type handling - cannot read byte array"
		);

		SmartAssert.assertSoft(serializer.canWrite(new byte[] {}),
				CoreMatchers.is(false),
				"Wrong content type handling. Can write byte array"
		);

		SmartAssert.assertSoft(serializer.canWrite("some string"),
				CoreMatchers.is(true),
				"Wrong content type handling. Cannot write string"
		);

		SmartAssert.assertSoft(serializer.canWrite(100L), CoreMatchers.is(true), "Wrong content type handling. Cannot write Long");

		SmartAssert.assertSoft(serializer.canWrite(Calendar.getInstance().getTime()),
				CoreMatchers.is(true),
				"Wrong content type handling. Cannot write Date"
		);
