...
public class DefaultErrorHandlerTest {
    ...
    public void testErrorHandlerClientError() throws IOException {
        HttpResponse response = getHttpResponse(404, "Not Found");
        handler.handle(request, response);
    }

    @Test(expected = RestEndpointServerException.class)
    public void testErrorHandlerServerError() throws IOException {
        HttpResponse response = getHttpResponse(500, "Internal Server Error");
        handler.handle(request, response);
    }

    @Test
    public void testHandlerInformationalResponse() throws IOException {
        HttpResponse response = getHttpResponse(100, "Continue");
        handler.handle(request, response);
    }

    @Test
    public void testErrorHandlerSuccessResponse() throws IOException {
        HttpResponse response = getHttpResponse(200, "Success");
        handler.handle(request, response);
    }

    @Test
    public void testHandlerRedirectionResponse() throws IOException {
        HttpResponse response = getHttpResponse(302, "Found");
        handler.handle(request, response);
    }
