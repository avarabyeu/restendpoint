...
public class HttpClientRestEndpoint implements RestEndpoint, Closeable {
    ...
    public final <RS> CompletableFuture<Response<RS>> post(String resource, MultiPartRequest request, Class<RS> clazz) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Unable to build post multipart request", e);
        }
        return executeInternal(post, new ClassConverterCallback<>(serializers, clazz));
    }
    ...
    @Override
    public final <RS> CompletableFuture<RS> deleteFor(String resource, Class<RS> clazz) {
    ...
    @Override
    public final <RS> CompletableFuture<RS> getFor(String resource, Class<RS> clazz) {
        return get(resource, clazz).thenApply(new BodyTransformer<>());
    }

    @Override
    public final <RS> CompletableFuture<Response<RS>> get(String resource, Type type) {
        HttpGet get = new HttpGet(spliceUrl(resource));
        return executeInternal(get, new TypeConverterCallback<>(serializers, type));
    }

    @Override
    public final <RS> CompletableFuture<RS> getFor(String resource, Type type) {
     ...
     */
    private URI spliceUrl(String resource) {
        try {
     ...
     * @return Absolute URL to the REST Resource including server and port
     */
    final URI spliceUrl(String resource, Map<String, String> parameters) {
        try {
            URIBuilder builder;
            if (!Strings.isNullOrEmpty(baseUrl)) {
